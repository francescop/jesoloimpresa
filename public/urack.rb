  ...
  module Handler
	...
	if options[:threads] > 2
      ...
      def self.serve(client, app, options)
        ...
        ulog("req: #{$requests} ip: #{env['REMOTE_ADDR']} pid: #{Process.pid} as: #{human_round(syscall(356).to_f/1024/1024)} MB => #{env['REQUEST_METHOD']} #{env['REQUEST_URI']} in #{Time.now-speed} secs [#{status}]#{' 
DISCONNECTED !!!' if disconnected}") if options[:logging]
      end
